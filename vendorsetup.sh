git clone --depth=1 https://github.com/neophyte-labs/vendor_xiaomi_vince -b 13 vendor/xiaomi/vince
git clone --depth=1 https://github.com/neophyte-labs/kernel_xiaomi_vince -b main kernel/xiaomi/vince
git clone --depth=1 https://github.com/k4ngcaribug/hardware_xiaomi -b 13 hardware/xiaomi
git clone --depth=1 https://github.com/k4ngcaribug/vendor_extra -b ten vendor/extra
git clone --depth=1 https://github.com/k4ngcaribug/vendor_extra -b dolby vendor/dolby/ds
git clone --depth=1 https://github.com/GrapheneOS/platform_external_Camera -b 13 platform/external/Camera
git clone --depth=1 https://github.com/iusmac/RevampedFMRadio -b qcom packages/apps/RevampedFMRadio
